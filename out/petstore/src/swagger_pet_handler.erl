...
get_request_spec('FindPetsByStatus') ->
    [
        {'status', #{
            source => qs_val,
            rules  => [{list, 'csv', [{type, 'binary'}, {enum, ['available', 'pending', 'sold']}, true
]}, true
, {required, true}]
        }}
    ];
get_request_spec('FindPetsByTags') ->
    [
        {'tags', #{
            source => qs_val,
...
get_request_spec('UpdatePetWithForm') ->
    [
        {'petId', #{
            source => binding,
            rules  => [{type, 'integer'}, {format, 'int64'}, true
...
get_response_spec('AddPet', 405) ->
    undefined;

get_response_spec('DeletePet', 400) ->
    undefined;

get_response_spec('FindPetsByStatus', 200) ->
    {'list', 'Pet'};

get_response_spec('FindPetsByStatus', 400) ->
    undefined;

get_response_spec('FindPetsByTags', 200) ->
    {'list', 'Pet'};

get_response_spec('FindPetsByTags', 400) ->
    undefined;

get_response_spec('GetPetById', 200) ->
    {'Pet', 'Pet'};

get_response_spec('GetPetById', 400) ->
    undefined;

get_response_spec('GetPetById', 404) ->
    undefined;

get_response_spec('UpdatePet', 400) ->
    undefined;

get_response_spec('UpdatePet', 404) ->
    undefined;

get_response_spec('UpdatePet', 405) ->
    undefined;

get_response_spec('UpdatePetWithForm', 405) ->
    undefined;

get_response_spec('UploadFile', 200) ->
